 ...
 * Capture monitor — สถานะ tracker_capture.exe (pid / uptime /
   ETW session), KPI 8 ตัว (events/sec, total events, tracked
   pids, file cache, key cache, CPU%, RAM, threads), live
   sparkline events/sec / CPU / RAM, per-kind bar chart
 ...
 * Auto-restart — ถ้า backend crash โดยที่คุณไม่ได้กด Stop
   tracker.exe จะ restart ให้ 3 ครั้ง (1s/2s/4s backoff)
 * Update check — ครั้งแรกที่เปิดเช็ค GitHub Releases เงียบ ๆ
   ถ้ามีรุ่นใหม่ขึ้น badge "vX.Y.Z available" ที่ header
   (offline ก็เปิดใช้งานได้ตามปกติ)
 * Per-launch auth token — อัตโนมัติ ไม่ต้องตั้งเอง
...
Q: อยากได้ auth token ที่ stable (ไม่เปลี่ยนทุกครั้งที่เปิด)
A: ตั้ง  set TRACKER_AUTH_TOKEN=mysecret  ก่อนเปิด tracker.exe
   tracker.exe จะ honour token นี้แทนการ generate ใหม่
   ใช้คู่กับ MCP_TRACKER_TOKEN ใน MCP client config
 ...
 * Per-launch bearer token — tracker.exe generate ใหม่ทุกครั้ง ส่งไป
   เบราว์เซอร์ผ่าน URL (?token=...) UI เก็บ localStorage แล้วลบจาก
   URL bar — ทุก /api/* + /ws/* request แนบ Authorization: Bearer
   ให้อัตโนมัติ ตั้ง TRACKER_AUTH_TOKEN เพื่อ pin token stable
 * No telemetry, no analytics, no cloud calls (ยกเว้น update check
   ตอนเปิดครั้งแรก ส่ง 1 GET ไป api.github.com — silent ถ้าออฟไลน์)
